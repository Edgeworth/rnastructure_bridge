...
bool Fold::parse(int argc, char** argv) {
  ...
  parser->addOptionFlagsWithParameters(constraintOptions,
                                       "Specify a constraints file to be applied. Default is to have no constraints applied.");
  ...
  parser->addOptionFlagsNoParameters(dnaOptions,
                                     "Specify that the sequence is DNA, and DNA parameters are to be used. Default is to use RNA parameters.");
  ...
  quickfoldOptions.push_back("-MFE");
  quickfoldOptions.push_back("--MFE");
  parser->addOptionFlagsNoParameters(quickfoldOptions,
                                     "Specify that only the minimum free energy structure is needed.  No savefiles can be generated.  This saves nearly half the calculation time, but provides less information.");

  // Add the simple_iloops option.
  vector <string> iloopmodeOptions;
  iloopmodeOptions.push_back("-y");
  iloopmodeOptions.push_back("-Y");
  parser->addOptionFlagsNoParameters(iloopmodeOptions,
                                     "Specify that the O(N^3) internal loop search should be used. This speeds up the calculation if large internal loops are allowed using the -l option.");
  ...
  parser->addOptionFlagsWithParameters(doubleOffsetOptions,
                                       "Specify a double-stranded offset file, which adds energy bonuses to particular double-stranded nucleotides. Default is to have no double-stranded offset specified.");
  ...
  parser->addOptionFlagsWithParameters(loopOptions,
                                       "Specify a maximum internal/bulge loop size. Default is 30 unpaired numcleotides.");
  ...
  parser->addOptionFlagsWithParameters(maxStructuresOptions,
                                       "Specify a maximum number of structures. Default is 20 structures.");
  ...
  parser->addOptionFlagsWithParameters(distanceOptions,
                                       "Specify a maximum pairing distance between nucleotides. Default is no restriction on distance between pairs.");
  ...
  parser->addOptionFlagsWithParameters(percentOptions,
                                       "Specify a maximum percent energy difference. Default is 10 percent (specified as 10, not 0.1).");
  ...
  parser->addOptionFlagsWithParameters(saveOptions,
                                       "Specify the name of a save file, needed for dotplots and refolding. Default is not to generate a save file.");
  ...
  parser->addOptionFlagsWithParameters(shapeOptions,
                                       "Specify a SHAPE restraints file to be applied. These constraints are pseudoenergy restraints. Default is to have no restraints applied.");
  ...
  parser->addOptionFlagsWithParameters(DshapeOptions,
                                       "Specify a differential SHAPE restraints file to be applied. These constraints are pseudoenergy restraints. Default is to have no restraints applied.");
  ...
  dmsOptions.push_back("-DMS");
  dmsOptions.push_back("--DMS");
  parser->addOptionFlagsWithParameters(dmsOptions,
                                       "Specify a DMS restraints file to be applied. These restraints are pseudoenergy constraints. Default is to have no restraints applied.");
  ...
  cmctOptions.push_back("--CMCT");
  parser->addOptionFlagsWithParameters(cmctOptions,
                                       "Specify a CMCT constraints file to be applied. These constraints are pseudoenergy constraints. Default is to have no constraints applied.");
  ...
  parser->addOptionFlagsWithParameters(shapeInterceptOptions,
                                       "Specify an intercept used with SHAPE restraints. Default is -0.6 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(shapeSlopeOptions,
                                       "Specify a slope used with SHAPE renstraints. Default is 1.8 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(DshapeSlopeOptions,
                                       "Specify a slope used with differential SHAPE restraints. Default is 2.11 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(singleOffsetOptions,
                                       "Specify a single-stranded offset file, which adds energy bonuses to particular single-stranded nucleotides. Default is to have no single-stranded offset specified.");
  ...
  parser->addOptionFlagsWithParameters(tempOptions,
                                       "Specify the temperature at which calculation takes place in Kelvin. Default is 310.15 K, which is 37 degrees C.");
  ...
  parser->addOptionFlagsWithParameters(bootstrapOptions,
                                       "Specify the number of bootstrap iterations to be done to retrieve base pair confidence. Defaults to no bootstrapping.");
  ...
  parser->addOptionFlagsWithParameters(shapeInterceptUnpairedOptions,
                                       "Specify an intercept used with unpaired SHAPE constraints. Default is 0 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(shapeSlopeUnpairedOptions,
                                       "Specify a slope used with unpaired SHAPE constraints. Default is 0 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(windowOptions,
                                       "Specify a window size. Default is determined by the length of the sequence.");
  ...
  parser->addOptionFlagsWithParameters(experimentalOptions,
                                       "Input text file with bonuses (in kcal) as a matrix. As with SHAPE, bonuses will be applied twice to internal base pairs, once to edge base pairs, and not at all to single stranded regions. Default is no experimental pair bonus file specified.");
  ...
  parser->addOptionFlagsWithParameters(experimentalOffsetOptions,
                                       "Specify an intercept (overall offset) to use with the 2D experimental pair bonus file. Default is 0.0 (no change to input bonuses).");
  ...
  parser->addOptionFlagsWithParameters(experimentalScalingOptions,
                                       "Specify a number to multiply the experimental pair bonus matrix by. Default is 1.0 (no change to input bonuses).");
  ...
  for (b_iter = 0; b_iter <= bootstrap; b_iter++) {
    ...
    if (error == 0 && applyConstraints) {
      ...
      if (error == 0 && experimentalFile != "") {
        constraintError = strand->ReadExperimentalPairBonus(experimentalFile.c_str(), experimentalOffset,
                                                            experimentalScaling);
        error = checker->isErrorStatus(constraintError);
      }
    ...
    if (error == 0) {
      ...
      int mainCalcError = strand->FoldSingleStrand(percent, maxStructures, windowSize, saveFile.c_str(), maxLoop,
                                                   quickfold);
      //int mainCalcError = strand->FoldSingleStrand( percent, maxStructures, windowSize, saveFile.c_str(), maxLoop, quickfold, simple_iloops);
