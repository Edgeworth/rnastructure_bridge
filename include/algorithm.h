...
void efn2(datatable* data, structure* ct, int structnum = 0, bool simplemb = false,
    const char* outputfile = NULL);  // energy calculator
...
// quickenergy indicates whether to determine the lowest free energy for the sequence without a
// structure
// quickstructure is a bool that will generate only the lowest free energy structure.  No savefiles
// can generated.
...
int dynamic(structure* ct, datatable* data, int cntrl6, int cntrl8, int cntrl9,
    ...
    bool quickstructure = false, bool simple_iloops = true, dp_state_t* dp_state = nullptr);
...
void fill(structure* ct, arrayclass& v, arrayclass& w, arrayclass& wmb, forceclass& fce, int& vmin,
    ...
    arrayclass* w2, arrayclass* wmb2, arrayclass* we, TProgressDialog* update = 0,
    int maxinter = 30, bool quickstructure = false, bool simple_iloops = true);

// The fill step of the dynamic programming algorithm for free energy minimization:
void fill(structure* ct, arrayclass& v, arrayclass& w, arrayclass& wmb, forceclass& fce, int& vmin,
    bool* lfce, bool* mod, integersize* w5, integersize* w3, bool qickenergy, datatable* data,
    arrayclass* w2, arrayclass* wmb2, TProgressDialog* update = 0, int maxinter = 30,
    bool quickstructure = false, bool simple_iloops = true);
...
void dynamic(structure* ct, datatable* data, int cntrl6, int cntrl8, int cntrl9, arrayclass* v,
    ...
    TProgressDialog* update = 0, bool quickenergy = false, char* savefile = 0, int maxinter = 30,
    bool quickstructure = false, bool simple_iloops = true);
...
void fill(structure* ct, arrayclass& v, arrayclass& vmb, arrayclass& vext, arrayclass& w,
    ...
    TProgressDialog* update = 0, int maxinter = 30, bool quickstructure = false,
...
void dotefn2(structure* ct, datatable* data, arrayclass* v, arrayclass* w, arrayclass* w2, int* w3,
    int* w5, short int** fce, bool* lfce, int vmin, dotarray* dots, TProgressDialog* PD = 0);
void calcpnum(
    dotarray* dots, int* pnum, int increment, short int numofbases, TProgressDialog* PD = 0);
...
short int getbestdot(dotarray* dots1, dotarray* dots2, structure* ct1, structure* ct2, short int i,
    short int j);  // return the best dot for base i
...
void opensav(char* filename, structure* ct, int cntrl6, int cntrl8,
    int cntrl9);  // opens a save file with information filled by
// fill algorithm
...
int trace(structure* ct, datatable* data, int ii, int ji, arrayclass* v, arrayclass* w,
    ...
    integersize* w5, bool* mod, arrayclass* we = NULL, integersize energy = 0, short open = 0,
    short pair = 0, bool quickstructure = false);
#else
int trace(structure* ct, datatable* data, int ii, int ji, arrayclass* v, arrayclass* w,
    arrayclass* wmb, arrayclass* w2, arrayclass* wmb2, bool* lfce, forceclass* fce, integersize* w3,
    integersize* w5, bool* mod, bool quickstructure = false);
#endif
...
void writehelixfile(
    char* filename, structure* ct, int StructureNumber);  // write a file with the helices
