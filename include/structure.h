...
// Single structure is a wrapper for the information associated with just a single structure, i.e.
// pairs, energy, and labels.
...
class structure  // this structure contains all the info for a structure
{
  ...
  short int pair[maxforce][2],npair,nforbid,forbid[maxforce][2];//arrays to hold lists of forced
  pairs or pairs forbidden
  ...
  short int neighbors[maxforce][maxneighborlength],nneighbors;//also NMR-derived index this from
  zero in both dimensions
  //regional NMR constraints:
  short int nregion,rmin_gu[maxregions],rmin_g_or_u[maxregions];
  short int
  rneighbors[maxregions][maxforce][maxneighborlength],rnneighbors[maxregions],start[maxregions],stop[maxregions];
  ...
  void sort();//sort the structures so that the lowest free energy structure is in position one
              //NOTE: This function sorts the the list of base pairs according to energy.  The
  energies (*energy) and basepairs (**basepr)
  ...
  void ReadSHAPE(const char *filename, std::string modifier="SHAPE", bool calculate=true, bool
  nosum=false);//Read SHAPE reactivity data from a file
  void ReadSHAPE(const char *filename, float SingleStrandThreshold, float
  ModificationThreshold);//Read SHAPE reactivity data from a file
  void ReadOffset(const char *SSOffset, const char *DSOffset);//Read Free Energy Offset Files.
  void ReadExperimentalPairBonus(const char *filename, double const experimentalOffset = 0.0, double
  const experimentalScaling = 1.0 );
  bool limitdistance;//toggle to indicate that there is a limit on the maximum distance between nucs
  in base pairs
  ...
  double SHAPEslope,SHAPEintercept;//values of slope and intercept for SHAPE data modification of
  pairing stability
  ...
  short int **SHAPEss_region;  //2-d short int array containing energy values for hairpin loop
  combinations
  ...
  int SHAPEss_calc(int index_i, int index_j);  //Returns pseudoenergy term for a hairpin loop using
  single stranded SHAPE data
  short int SHAPEss_give_value(int index);  //Returns the single stranded SHAPE pseudo energy for a
  given nucleotide
...
public:
  ...
  //! \param structurenumber is an int that gives the structure number being indexed, this is one
  //! indexed.
  ...
  //! This function requires that an energy calculation has been performed.  It does not do an
  //! eenrgy calculation.
  //! \param structurenumber is an int that gives the structure number being indexed, this is one
  //! indexed.
  //! \return an int that gives the energy in kcal/mol*conversionfactor, where conversionfactor is
  //! set in /src/defines.h.
  ...
  //! \param structurenumber is an int that provides from which structure the pair should be
  //! removed.  This is one-indexed.
  ...
  //! \param structurenumber is the index to which structure will hold the label.  This is
  //! one-indexed.
  ...
  //! \param structurenumber is an int that gives the structure number being indexed, this is one
  //! indexed.
  //! \param energy is an int that sets the energy in kcal/mol*conversionfactor, where
  //! conversionfactor is set in /src/defines.h.
  ...
  //! This function must be called after reading a sequence and before any template information is
  //! read or written.
  //! This mechanism is orthogonal to the functions AddForbiddenPair, GetForbiddenPair5, and
  //! GetForbiddenPair3.  It exists for the convenience of coding functiopns that need to forbid a
  ...
  //! These nucleotides can only be unpaired, at the end of a helix, in a GU pair, or adjacent to a
  //! GU pair.
  //! \param i is an int that indicates the nucleotide position, one indexed.
  ...
  inline bool DistanceLimited() { return limitdistance; }

  //! Get a nucleotide that must be base paired.

  //! \return An int that gives the nucleotide position, one indexed.
  //! \param i is an int that gives the constraint number, which should be between 0 and
  //! GetNumberofDoubles()-1, inclusive.
  int GetDouble(int i);
  ...
  //! \param i is an int that gives the constraint number, which should be between 0 and
  //! GetNumberofForbiddenPairs()-1, inclusive.
  ...
  //! \return An int that gives the nucleotide position, one indexed.
  //! \param i is an int that gives the constraint number, which should be between 0 and
  //! GetNumberofMofidied()-1, inclusive.
  ...
  inline int GetPairingDistanceLimit() { return maxdistance; }

  //! Get a nucleotide that must be single stranded.

  //! \return An int that gives the nucleotide position, one indexed.
  //! \param i is an int that gives the constraint number, which should be between 0 and
  //! GetNumberofSingles()-1, inclusive.
  int GetSingle(int i);
  ...
  //! the file.  The default, falase, is to overwrite any existing file.
  ...
  //! A non-zero return indicates and error in reading the file.  The value of the return is the
  //! line number that contains an error.
  //! \param ctfile is a pointer to a Null-terminated cstring that gives the filename, including any
  ...
  //! \param seqfile is a const char pointer to a cstring that gives the filename, including any
  //! path information.
  ...
  //! The file will be decipherable only if there are no pseudoknots in the structure.  There is no
  ...
  //! \param structurenumber is an int that is the index to which structure should be removed.  This
  //! is one indexed.
  ...
  // void ReadSHAPE(const char *filename, bool calculate=true);//Read SHAPE reactivity data from a
  // file
  ...
  short int min_gu, min_g_or_u;  // NMR-derived constraint variables
  short int neighbors[maxforce][maxneighborlength],
      nneighbors;  // also NMR-derived index this from zero in both dimensions
  // regional NMR constraints:
  short int nregion, rmin_gu[maxregions], rmin_g_or_u[maxregions];
  short int rneighbors[maxregions][maxforce][maxneighborlength], rnneighbors[maxregions],
      start[maxregions], stop[maxregions];
  // microarray type constraints:
  short int nmicroarray, microstart[maxregions], microstop[maxregions], microunpair[maxregions];
  bool* fcedbl;  // pointer to a 2-D array used in Dynalign to track nucleotides that must be
                 // double-stranded
