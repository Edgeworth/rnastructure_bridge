...
void pkHelix::addHelixToStructure(structure* st, int structureNumber) {
  ...
  st->SetEnergy(structureNumber, st->GetEnergy(structureNumber) +
                                 energy);  //structure object parameter defining overall energy of the folded RNA
  ...
  for (int i = 0; i < length; i += 2) {
    st->SetPair(helix[i + 1], helix[i],
                structureNumber);  //basepr is list of base pairs in structure object. Each element in this list holds its basepairing partner, ie base pair 10-20 is basepair[10]=20.
