...
#ifdef _WINDOWS_GUI
#include "../RNAstructure_windows_interface/stdafx.h"
#endif
...
extended_double operator+(const extended_double& ext1, const extended_double& ext2) {
  ...
  switch (ext1.var_cap + ext2.var_cap) {
    ...
    case 1:
      ...
      ADD.var = (ext1.var + ext2.var / extended_double::cap) * ext1.var_cap +
                (ext1.var / extended_double::cap + ext2.var) * ext2.var_cap;
...
extended_double operator*(const extended_double& ext1, const extended_double& ext2) {
  ...
  switch (ext1.var_cap + ext2.var_cap) {
    case 2: //no need to check scaling since MULT cannot go below cap limit in this case
      MULT.var =
          ext1.var * ext2.var * extended_double::cap; //extra scaling term must be multiplied on to keep correct units
      MULT.var_cap = 1;
      break;
...
bool operator<(const extended_double& ext1, const double& ext2) {
  if (ext1.var_cap == 0) return ext1.var < ext2;
    ...
    return ext1.var < 0;  //if ext1 is scaled, then the only way it can be smaller than a double is if ext1 is negative.
...
bool operator<(const extended_double& ext1, const extended_double& ext2) {
  switch (ext1.var_cap + ext2.var_cap) {
    case 2:
      return ext1.var < ext2.var;
    case 1:
      if (ext1.var_cap == 1) return ext1.var < 0;
