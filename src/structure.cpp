...
structure::structure(int structures) {
  ...
  // initialize values for SHAPE slope and intercept as zero for both double and single stranded
  // restraints.
  SHAPEslope_ss = 0;
  SHAPEintercept_ss = 0;
  ...
  ssoffset =
      false;  // by default, a structure does not have a single stranded offset read from disk
...
int structure::GetNumberofStructures() { return (int)arrayofstructures.size(); }

// Get the pairing partner of i in structure structurenumber
int structure::GetPair(int i, int structurenumber) {

  // Refer to structurenumber-1 so that the underlying zero indexing works with the 1 indexing used
  // in the rest of the
  // software.
...
string structure::GetSequenceLabel() { return sequencelabel; }
...
void structure::RemovePair(int i, int structurenumber) {
  ...
  if (arrayofstructures[structurenumber - 1].basepr[i] != 0) {
    ...
    arrayofstructures[structurenumber - 1]
        .basepr[arrayofstructures[structurenumber - 1].basepr[i]] = 0;
...
void structure::SetSequenceLabel(string label) { sequencelabel = label; }
...
void structure::AddDouble(int i) { doublestranded.push_back(i); }
...
void structure::AddGUPair(int i) { GUpair.push_back(i); }

// Add a nucleotide to the list of those accessible to traditional chemical modification.
void structure::AddModified(int i) { modified.push_back(i); }
...
void structure::AddSingle(int i) { singlestranded.push_back(i); }

//! Get a nucleotide that must be base paired.
int structure::GetDouble(int i) { return doublestranded[i]; }

// Get a nucleotide that must not be in a specific pair.
int structure::GetForbiddenPair5(int i) { return forbid5[i]; }
...
int structure::GetGUpair(int i) { return GUpair[i]; }

// Get a nucleotide that is accessible to chemical modification.
int structure::GetModified(int i) { return modified[i]; }

// Get the number of nucleotides forced to be double-stranded.
int structure::GetNumberofDoubles() { return doublestranded.size(); }

// Get the number of pairs that are forbidden.
int structure::GetNumberofForbiddenPairs() { return forbid5.size(); }

// Get the number of Us forced to be in GU pairs.
int structure::GetNumberofGU() { return GUpair.size(); }

// Get the number of nucleotides that are accessible to chemical modification.
int structure::GetNumberofModified() { return modified.size(); }

// Get the number of nucleotides forced to be single-stranded.
int structure::GetNumberofSingles() { return singlestranded.size(); }

// Get the number of pairs that are constrained to occur.
int structure::GetNumberofPairs() { return pair5.size(); }

// Get a nucleotide that must be in a specific pair.
int structure::GetPair5(int i) { return pair5[i]; }

// Get a nucleotide that must be in a specific pair.
int structure::GetPair3(int i) { return pair3[i]; }

//<<<<<<< structure.cpp
// Get a nucleotide that must be single stranded.
int structure::GetSingle(int i) { return singlestranded[i]; }
//=======
// This function calculates the pseudoenergy for a given reactivity data. It changes the calculation
// depending on the modifier specified, giving either the log-likelihood-ratio of the
// unpaired/paired
...
/*double structure::CalculatePseudoEnergy(double data, std::string modifier, double slope, double
intercept) {
...
// By default, the columns for indicies are only 5 characters wide, so this is a problem for
// sequences > 9,999 nucs.
// Now, when sequences are >9,999 nucs, columns are 6 characters wide.  (The code is written yet for
// sequences > 99,999
// nucs.)
void structure::ctout(const char* ctoutfile, bool append) {
  ...
  for (count = 1; count <= (GetNumberofStructures()); count++) {
    ...
    strcpy(line, "");
    if (numofbases > 9999)
      sprintf(line, "%6i", numofbases);
    else
      sprintf(line, "%5i", numofbases);
    ...
    if (GetEnergy(count) != 0) {
      ...
      if (conversionfactor == 10)
        ...
        sprintf(number, "%f", (float(GetEnergy(count))) / conversionfactor);
    ...
    for (i = 1; i < numofbases; i++) {
      ...
      // else {
      if (numofbases > 9999)
        sprintf(line, "%6i%2c%8i%6i%6i%6i\n", i, nucs[i], (i - 1), (i + 1), GetPair(i, count),
            hnumber[i]);
      else
...
long structure::openct(const char* ctfile) {
  ...
  // First read the first item in the file.  If this is -100, then the file was flagged as a
  // CCT-formatted file.  (A
  // more compact format.)
  in >> count;
  j = 0;
  ...
  if (count == -100) {
    ...
    for (int numofstructures = 1; numofstructures <= count; numofstructures++) {
      ...
      if (numofstructures == 1) { SetSequenceLabel(header); }
    ...
    // RMW 2015-03-12: count can be an invalid number, even negative, if the file format is not
    // correct.
    ...
    // I've added the following try/catch to prevent this, but a better solution would be to
    // add better file-type detection.
    cout << "Invalid file format: Negative number of bases." << endl;
    return -1;
  } else {  // this is a ct file:

    // FOR NOW, DISABLE READING CT FILES WITH STACKING INFO: THIS SEEMS TO BE A PROBLEM WITH THE
    // WEBSERVER BECAUSE THERE
    // ARE MANY FORMATS IN CURRENT USE
    ...
    for (int numofstructures = 1; !in.eof(); ++(numofstructures)) {
      ...
      for (count = 1; count <= ((numofbases)); count++) {
        ...
        nucs[count] = base[0];
        if (base[0] == 'A' || base[0] == 'a')
          numseq[count] = 1;
        else if (base[0] == 'C' || base[0] == 'c')
          numseq[count] = 2;
        else if (base[0] == 'G' || base[0] == 'g')
          numseq[count] = 3;
        else if (base[0] == 'U' || base[0] == 'u' || base[0] == 'T' || base[0] == 't')
          numseq[count] = 4;
        else if (base[0] == 'I')
          numseq[count] = 5;
        else
        ...
        in >> basepair;
        if (basepair > count)
          SetPair(count, basepair, GetNumberofStructures());  // read base pairing info
        // Check the base pairing is consistent if this is the 3' nucleotide in a pair
        if (GetPair(count, GetNumberofStructures()) < count &&
            GetPair(count, GetNumberofStructures()) != 0) {
...
int structure::openseq(const char* seqfile) {
  ...
  if ((se = fopen(seqfile, "r")) == NULL) { return 0; }
  ...
  if (first == ';') {
    ...
    if (nucleotides == 0) {
      // TODO: try to properly report this error via return codes.
      cerr << "Error reading seq file. The file contained no nucleobases." << endl;
      return 0;
    }
    ...
    if (nucleotides == 0) {
      cerr << "Error reading FASTA file: The file did not contain any nucleobases." << endl;
      return 0;
    }
...
// Note:  This function assumes that there are no pseudoknots, which would make the output
// un-parsable
...
void structure::AddStructure() {
  ...
  if (arrayofstructures.size() == 1) { arrayofstructures[0].ctlabel = sequencelabel; }
}
...
void structure::RemoveLastStructure() { arrayofstructures.pop_back(); }

// This function sizes the vectors ctlable and energy to the right size for the maximum expected
// number of structures.
// This function is called multiple times if the number of structures is going the exceed the
// allocatedstructures.
...
//! This function is for debugging.  It checks each pair in each structure to look for
//! inconsistencies.
...
// this allocates space in an array that is used for applying an equilibrium constant for missing
// specific pairs
...
double structure::Potential(double data, std::vector<std::vector<double>> params, double kT) {
  // params[0] is for paired, params[0] for unpaired...params[][j], j=0,1,2 for shape, loc scale of
  // component 1
...
void structure::ReadProbabilisticPotentialParams() {
  ...
  // Start reading distribution parameters....note that this code is ugly, can be collapsed into a
  // single parameter
  // array
  // SHAPE
...
// probabilities given a reactivity distribution per modifier, or the "classic" Deigan et al. bonus
// term when no modifier or an unrecognized modifier is provided.
double structure::CalculatePseudoEnergy(
    double data, std::string modifier, double slope, double intercept) {
  std::vector<std::vector<double>> params;
...
void structure::ReadSHAPE(const char* filename, std::string modifier, bool calculate) {
  ...
  if (!SHAPEFileRead) {
    ...
    ////This function reads a SHAPE reactivity datafile and saves the data for a linear penalty.
    // calculate (default true) indicate whether these data are being read for folding.  (false
    // means
    // the raw values need to be stored.)
    // void structure::ReadSHAPE(const char *filename, std::string modifier, bool calculate, bool
    // nosum) {
    //	ifstream in;
    //	int position;
    //	double data;
  ...
  while (!in.eof()) {
    ...
    if (position <= numofbases) {
      if (calculate) {
        ...
        SHAPEssnew[position] +=
            CalculatePseudoEnergy(data, modifier, SHAPEslope_ss, SHAPEintercept_ss);
        if (SHAPEnew[position] != 0) counts[position] += 1;
      } else {
        SHAPE[position] = data;
...
// This function must be called ofter reading SHAPE files, if read, because the same infrastructure
// is used.
...
void structure::ReadOffset(const char* SSOffset, const char* DSOffset) {
  ...
  if (SSOffset != NULL) {
    ...
    while (!in.eof()) {
      // read and parse all data
      // required format is rows with sequence position followed by reactivity
      // multiply the specified free energy change by converstionfactor, the factor by which
      // energies in kcal/mol are
      // multipled
  ...
  // fills 2-d array with pseudo energy terms for loops from i-j using ss SHAPE parameters or
  // offsets
...
short int structure::SHAPEss_give_value(int index) {
  if (shaped) {
    if (index > numofbases)
      return (short int)SHAPEss[index - numofbases];
    else
...
// this is the function that will return a pseudo energy term for hairpin loops based off of SHAPE
// data
...
// This function reads a SHAPE reactivity datafile and parse the data into single-stranded amd
// chemical modification
// constraints.
// This function is largely depracated by the pseudo-free energy approach.  It is still available
// for experimentation.
void structure::ReadSHAPE(
    const char* filename, float SingleStrandThreshold, float ModificationThreshold) {
  ifstream in;
  int position;
  float data;
...
void structure::ReadExperimentalPairBonus(
    const char* filename, double const experimentalOffset, double const experimentalScaling) {
...
int ecompare(const void* i, const void* j) { return (**((short int**)i) - **((short int**)j)); }
