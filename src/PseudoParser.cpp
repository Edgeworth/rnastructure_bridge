...
void FillMismatch(RNA* rna, int strnum) {
  ...
  for (int i = 1; i <= rna->GetSequenceLength() - 8; ++i) {
    ...
    if (rna->GetPair(i, strnum) > i) {
      ...
      if (rna->GetPair(i + 1, strnum) == 0 && rna->GetPair(rna->GetPair(i, strnum) - 1, strnum) == 0 &&
          rna->GetPair(i + 2, strnum) == j - 2 &&
          isCanonical(rna->GetNucleotide(i + 1), rna->GetNucleotide(rna->GetPair(i, strnum) - 1))) {
      ...
      if (rna->GetPair(rna->GetPair(i, strnum) - 1, strnum) == 0 &&
          rna->GetPair(rna->GetPair(i, strnum) - 2, strnum) == 0 && rna->GetPair(i + 1, strnum) == 0 &&
          rna->GetPair(i + 2, strnum) == 0 && rna->GetPair(i + 3, strnum) == j - 3 &&
          isCanonical(rna->GetNucleotide(i + 1), rna->GetNucleotide(rna->GetPair(i, strnum) - 1)) &&
          isCanonical(rna->GetNucleotide(i + 2), rna->GetNucleotide(rna->GetPair(i, strnum) - 2))) {
...
void RemoveIsolatedPairs(RNA* rna, int strnum) {
  ...
  for (int i = 1; i <= rna->GetSequenceLength(); ++i) {
    ...
    if (rna->GetPair(i, strnum) > i) {
      ...
      while (rna->GetPair(i + 1, strnum) == j - 1 || rna->GetPair(i + 2, strnum) == j - 1 ||
             rna->GetPair(i + 1, strnum) == j - 2) {
      ...
      for (pknum = 0; pknum < pk.size(); ++pknum) {
        ...
        for (itnum = 0; itnum < pk[pknum].GetNumberIntact(); ++itnum) {
          ...
          for (k = i + 1; k < pairsBroken[i]; ++k) {
            ...
            if (rna->GetPair(k, strnum) != 0 &&
                (rna->GetPair(k, strnum) < i || rna->GetPair(k, strnum) > pairsBroken[i])) {
...
void
DistanceCounter(RNA* rna, vector <Pseudoknot>& pk, vector<int>& rnacopy, vector<int>& UNcount, vector<int>& IBcount,
                vector <vector<int>>& SHLcount, int strnum) {
  /////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (unsigned int pknum = 0; pknum < pk.size(); ++pknum) {
    ...
    while (rnacopy[b + 1] == rnacopy[b] - 1 || (rnacopy[b + 1] == rnacopy[b] - 2 && rnacopy[rnacopy[b] - 1] == 0) ||
           (rnacopy[b + 2] == rnacopy[b] - 1 && rnacopy[b + 1] == 0)) {
    ...
    while (rnacopy[e - 1] == rnacopy[e] + 1 || (rnacopy[e - 1] == rnacopy[e] + 2 && rnacopy[rnacopy[e] + 1] == 0) ||
           (rnacopy[e - 2] == rnacopy[e] + 1 && rnacopy[e - 1] == 0)) {
    ...
    while (b < e) {
      ...
      if (rnacopy[b] == 0) {
        ...
        while ((rnacopy[b + 1] == rnacopy[b] - 1 && rnacopy[b] - 1 != 0) ||
               (rnacopy[b + 1] == rnacopy[b] - 2 && rnacopy[rnacopy[b] - 1] == 0 && rnacopy[b] - 1 != 0 &&
                rnacopy[b] - 2 != 0) ||
               (rnacopy[b + 2] == rnacopy[b] - 1 && rnacopy[b + 1] == 0 && rnacopy[b] - 1 != 0)) {
...
void ReadPseudoParam(double* pseudo_param) {
  ...
  if (!pseudoConst.is_open()) {
    cerr
        << "File \"pseudconst.dat\" with pseudoknot penalty constants is not found. This file has to be placed in \"~/RNAstructure/data_tables/\"."
        << endl;
    ...
    for (unsigned int pknum = 0; pknum < pk.size(); ++pknum) {
      ...
      for (i = 1; i <= 15; ++i) {
        if (SHLcount[pknum][i] != 0 && i < 15) {
          cout << "								   " << SHLcount[pknum][i] << " count(s) of " << i << "nt spanning helixe(s)"
               << endl;
        }
        else if (SHLcount[pknum][i] != 0 && i == 15) {
          cout << "								   " << SHLcount[pknum][i] << " count(s) of "
               << "spanning helixe(s) with length longer then 15nt" << endl;
        }
      }
      cout << endl << endl;
  ...
  if (rna->ContainsPseudoknot(strnum)) {
    ...
    for (unsigned int k = 0; k < pk.size(); ++k) {
      ...
      if (UNcount[k] == 0 && IBcount[k] == 0) {
        ...
        energyPenalty = energyPenalty + (double) (((
            P1 * log(a[0] * a[0] * ((double) UNcount[k]) + a[1] * a[1] * ((double) IBcount[k])) +
            P2 * log(SHLcountTOTAL))));
      }
    }
  }
...
double CalculatePKhelixEnergy(RNA* ct, int strnum, datatable* data) {
  ...
  for (int i = 1;
       i <= ct->GetSequenceLength() - 7; ++i) {//for every nucleotide up to 7 nucleotides till the end of the sequence
    if (i < ct->GetPair(i, strnum) && ct->GetPair(i, strnum) != 0) {//if it is a 5' to 3' pair
      ...
      if (ct->GetPair(i + 1, strnum) == ct->GetPair(i, strnum) - 1) {
        ...
        energy += erg1(i, ct->GetStructure()->GetPair(i, strnum), i + 1, ct->GetStructure()->GetPair(i, strnum) - 1,
                       ct->GetStructure(), data);

        //calculate AU penalty if it is opening or closing pair
        //Check if it is the first nucleotide
        if (i == 1) energy += penalty2(i, ct->GetPair(i, strnum), data);
          //check if it is the last nucleotide
        else if (ct->GetPair(i, strnum) == ct->GetSequenceLength()) energy += penalty2(i, ct->GetPair(i, strnum), data);
          ...
          energy += penalty2(i, ct->GetPair(i, strnum), data);
        }
...
double
CalculateFreeEnergywithPseudoknot(RNA* rna, int strnum, double P1, double P2, double* pseud_param, datatable* data,
                                  bool UseSimpleMBLoopRules, bool fillMismatch, bool remIsolatedPairs) {
  ...
  if (rna->ContainsPseudoknot(strnum)) {
    ...
    double PseudPenalty = pkEnergyCalc(rna, pseud_param, P1, P2, strnum);
