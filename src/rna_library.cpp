...
int opendat(const char* loop2, const char* stackf, const char* tstackh, const char* tstacki,
    const char* tloop, const char* miscloop, const char* danglef, const char* int22,
    const char* int21, const char* coax, const char* tstackcoax, const char* coaxstack,
    const char* tstack, const char* tstackm, const char* triloop, const char* int11,
    const char* hexaloop, const char* tstacki23, const char* tstacki1n, datatable* data) {
  ...
  if ((check = fopen(miscloop, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(loop2, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(stackf, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstackh, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstacki, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tloop, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(danglef, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(int22, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(int21, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(triloop, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(coax, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstackcoax, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(coaxstack, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstack, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstackm, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(hexaloop, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstacki23, "r")) == NULL) { return 0; }

  fclose(check);

  if ((check = fopen(tstacki1n, "r")) == NULL) { return 0; }
  ...
  for (count = 1; count <= 4; count++) {
    ml1 >> temp;
    (data->poppen[count]) = (int)round(temp * conversionfactor);
  ...
  data->eparam[4] = 0;
  ml1 >> temp;
  data->eparam[5] = (short)round(temp * conversionfactor);  // constant multi-loop penalty

  ml1 >> temp;
  data->eparam[6] = (short)round(temp * conversionfactor);
  ...
  if (ml1.peek() == EOF) {
    ...
    data->efn2a = (short)round(temp * conversionfactor);  // constant multi-loop penalty for efn2

    ml1 >> temp;
    data->efn2b = (short)round(temp * conversionfactor);
        ...
        for (k = 0; k <= 5; k++) {
          if ((i == 0) || (j == 0) || (k == 0)) {
            ...
            if (strcmp(lineoftext, ".")) {
              data->dangle[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->dangle[i][j][k][l] = INFINITE_ENERGY;
          }
  ...
  for (i = 1; i <= 30; i++) {
    ...
    lo1 >> lineoftext;
    if (strcmp(lineoftext, ".")) {
      data->inter[i] = (short)round(conversionfactor * (atof(lineoftext)));
    } else
      data->inter[i] = INFINITE_ENERGY;

    // cout <<"inter = "<<data->inter[i]<<"\n";
    lo1 >> lineoftext;
    if (strcmp(lineoftext, "."))
      data->bulge[i] = (short)round(conversionfactor * (atof(lineoftext)));
    else
      data->bulge[i] = INFINITE_ENERGY;

    // cout <<"bulge = "<<data->bulge[i]<<"\n";
    lo1 >> lineoftext;
    if (strcmp(lineoftext, ".")) {
      data->hairpin[i] = (short)round(conversionfactor * (atof(lineoftext)));
    } else
      data->hairpin[i] = INFINITE_ENERGY;
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0)) {
            ...
            st1 >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->stack[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->stack[i][j][k][l] = INFINITE_ENERGY;
          }
        }
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0)) {
            ...
            th1 >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->tstkh[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstkh[i][j][k][l] = INFINITE_ENERGY;
          }
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          ...
          //}
            ...
            if (strcmp(lineoftext, ".")) {
              data->tstki[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstki[i][j][k][l] = INFINITE_ENERGY;
          }
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          ...
          //}
            ...
            if (strcmp(lineoftext, ".")) {
              data->tstki23[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstki23[i][j][k][l] = INFINITE_ENERGY;
          }
          // cout <<"stack "<<i<<" "<<j<<" "<<k<<" "<<l<<"  "<<data->tstki[i][j][k][l]<<"\n";
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          ...
          //}
            ...
            if (strcmp(lineoftext, ".")) {
              data->tstki1n[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstki1n[i][j][k][l] = INFINITE_ENERGY;
          }
          // cout <<"stack "<<i<<" "<<j<<" "<<k<<" "<<l<<"  "<<data->tstki[i][j][k][l]<<"\n";
  ...
  for (count = 1; count <= maxtloop && !tl1.eof(); count++) {
    ...
    strcpy(base + 1, "\0");
    data->tloop[data->numoftloops][0] = data->tloop[data->numoftloops][0] + 5 * tonumi(base);
    // cout << base << "\n";
    // cout << data->tloop[data->numoftloops][0] << "\n";
    strcpy(base, lineoftext + 2);
    strcpy(base + 1, "\0");
    data->tloop[data->numoftloops][0] = data->tloop[data->numoftloops][0] + 25 * tonumi(base);
    // cout << base << "\n";
    // cout << data->tloop[data->numoftloops][0] << "\n";
    strcpy(base, lineoftext + 3);
    strcpy(base + 1, "\0");
    data->tloop[data->numoftloops][0] = data->tloop[data->numoftloops][0] + 125 * tonumi(base);
    // cout << base << "\n";
    // cout << data->tloop[data->numoftloops][0] << "\n";
    strcpy(base, lineoftext + 4);
    strcpy(base + 1, "\0");
    data->tloop[data->numoftloops][0] = data->tloop[data->numoftloops][0] + 625 * tonumi(base);
    // cout << base << "\n";
    // cout << data->tloop[data->numoftloops][0] << "\n";
    strcpy(base, lineoftext + 5);
    strcpy(base + 1, "\0");
    data->tloop[data->numoftloops][0] = data->tloop[data->numoftloops][0] + 3125 * tonumi(base);
    // cout << base << "\n";
    // cout << data->tloop[data->numoftloops][0] << "\n";
    tl1 >> temp;
    data->tloop[data->numoftloops][1] = (short)round(conversionfactor * temp);
    ...
    for (j = 1; j <= 4; j++) {
      for (k = 1; k <= 4; k++) {
        for (l = 1; l <= 4; l++) {
          for (m = 1; m <= 4; m++) {
            in1 >> temp;
            data->iloop22[a][b][c][d][j][l][k][m] = (short)round(conversionfactor * temp);

            // no longer need to store the reverse order at same time because
            // the tables contain redundancy:
            // data->iloop22[d][c][b][a][m][k][l][j] =round(conversionfactor*temp);
    ...
    for (e = 1; e <= 4; e++) {
      ...
      for (c = 1; c <= 4; c++) {
        for (j = 1; j <= 6; j++) {
          ...
          for (d = 1; d <= 4; d++) {
            in2 >> temp;
            data->iloop21[a][b][c][d][e][f][g] = (short)round(conversionfactor * temp);
            // cout << a<<" "<<b<<" "<<c<<" "<<d<<" "<<e<<" "<<f<<" "<<g<<"\n";
            // cout << temp<<"\n";
  ...
  for (count = 1; count <= maxtloop && !tri.eof(); count++) {
    ...
    tri >> temp;
    data->triloop[data->numoftriloops][1] = (short)round(conversionfactor * temp);
  ...
  for (count = 1; count <= maxtloop && !hl1.eof(); count++) {
    ...
    strcpy(base, lineoftext + 1);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 5 * tonumi(base);
    strcpy(base, lineoftext + 2);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 25 * tonumi(base);
    strcpy(base, lineoftext + 3);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 125 * tonumi(base);
    strcpy(base, lineoftext + 4);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 625 * tonumi(base);
    strcpy(base, lineoftext + 5);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 3125 * tonumi(base);
    strcpy(base, lineoftext + 6);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 15625 * tonumi(base);
    strcpy(base, lineoftext + 7);
    strcpy(base + 1, "\0");
    data->hexaloop[data->numofhexaloops][0] =
        data->hexaloop[data->numofhexaloops][0] + 78125 * tonumi(base);
    hl1 >> temp;
    data->hexaloop[data->numofhexaloops][1] = (int)round(conversionfactor * temp);
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0)) {
            ...
            if (strcmp(lineoftext, ".")) {
              data->coax[j][i][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->coax[j][i][k][l] = INFINITE_ENERGY;
  ...
  for (i = 0; i <= 5; i++) {
    ...
    for (k = 0; k <= 5; k++) {
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0) || (i == 5) || (j == 5) || (k == 5) ||
              (l == 5)) {
            data->tstackcoax[i][j][k][l] = 0;
          } else {
            co2 >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->tstackcoax[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstackcoax[i][j][k][l] = INFINITE_ENERGY;
          }
        }
      }
    }
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0) || (i == 5) || (j == 5) || (k == 5) ||
            ...
            co3 >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->coaxstack[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->coaxstack[i][j][k][l] = INFINITE_ENERGY;
          }
        }
      }
    }
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          ...
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0)) {
            ...
            st2 >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->tstack[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstack[i][j][k][l] = INFINITE_ENERGY;
          }
          // cout <<"stack "<<i<<" "<<j<<" "<<k<<" "<<l<<"  "<<data->tstki[i][j][k][l]<<"\n";
      ...
      for (j = 0; j <= 5; j++) {
        for (l = 0; l <= 5; l++) {
          if ((i == 0) || (j == 0) || (k == 0) || (l == 0)) {
            ...
            tsm >> lineoftext;
            if (strcmp(lineoftext, ".")) {
              data->tstkm[i][j][k][l] = (short)round(conversionfactor * (atof(lineoftext)));
            } else
              data->tstkm[i][j][k][l] = INFINITE_ENERGY;
          }
          // cout <<"stack "<<i<<" "<<j<<" "<<k<<" "<<l<<"  "<<data->tstkh[i][j][k][l]<<"\n";
  ...
  for (i = 1; i <= 6; i++) {  // read each row of table
    ...
    for (b = 1; b <= 4; b++) {
      for (j = 1; j <= 6; j++) {
        ...
        for (e = 1; e <= 4; e++) {
          i11 >> temp;
          data->iloop11[a][b][c][d][e][f] = (short)round(conversionfactor * temp);
        }
      }
    }
  }
...
// The pseudo energy is applied twice for each nuc in interior pair and once for each nuc in
// terminal pair.
...
integersize erg1(int i, int j, int ip, int jp, structure* ct, datatable* data) {
  ...
  if ((i == (ct->GetSequenceLength())) || (j == ((ct->GetSequenceLength()) + 1))) {
    ...
    if (ct->experimentalPairBonusExists) {
      energy += 0.5 * (ct->EX[i][j] + ct->EX[j][i]) + 0.5 * (ct->EX[ip][jp] + ct->EX[jp][ip]);
    }
...
integersize erg2(int i, int j, int ip, int jp, structure* ct, datatable* data, char a, char b) {
  ...
  if (size1 == 1)
    SHAPEss_energy += ct->SHAPEss_give_value(i + 1);
  else if (size1 != 0)
    SHAPEss_energy += ct->SHAPEss_calc(i + 1, ip - 1);

  if (size2 == 1)
    SHAPEss_energy += ct->SHAPEss_give_value(j - 1);
  else if (size2 != 0)
    SHAPEss_energy += ct->SHAPEss_calc(jp + 1, j - 1);
    ...
    if (size == 1) {
      ...
      if (size1 == 1) {
        ...
        while (ct->numseq[k] == ct->numseq[i + 1]) {
          ...
          // During suboptimal structure prediction (where sequence is doubled), make sure this
          // doesn't walk from one
...
integersize erg2in(int i, int j, int ip, int jp, structure* ct, datatable* data, char a, char b) {
  ...
  // size1 = ip-i-1;
  // size2 = j - jp - 1;
  if (size1 != 0 && size2 != 0) {  // internal loop
    // size = size1 + size2;
    lopsid = abs(size1 - size2);
...
integersize erg3(int i, int j, structure* ct, datatable* data, char dbl) {
  ...
  if (ct->numseq[i] == 3 && ct->numseq[j] == 4 && size >= 4) {
  ...
  if (size == 3)
    ...
    return (energy + data->cint + size * data->cslope);
}
...
//	require that the backbone continues directly between j and ip without a nucleotide in
//	a canonical pair
// k (determined by the function called) indicates the intervening nuc in intervening mismatch that
// is not sandwiched
// between
//	j and ip
...
integersize ergcoaxinterbases1(int i, int j, int ip, int jp, int k, int l, datatable* data) {
  ...
  return data->tstackcoax[j][i][l][k] + data->coaxstack[jp][ip][k][l];
}
...
integersize ergcoaxinterbases2(int i, int j, int ip, int jp, int k, int l, datatable* data) {
  ...
  return data->tstackcoax[jp][ip][k][l] + data->coaxstack[j][i][l][k];
}

// These functions are used by ergmulti to deconvolute the base pair code back into nucs
int decon1(int x) { return (int)(floor(((float)(x)) / 10) - 1); }
int decon2(int x) { return x - (decon1(x) + 1) * 10 - 1; }
...
integersize ergmulti(int st, int ip, structure* ct, datatable* data, bool simplemb) {
  ...
  while (i != ip || count == 0) {
    i++;
    if (ct->GetPair(i, st) != 0) { i = ct->GetPair(i, st); }
    ...
    for (i = 0; i + size < count + 4; i++) {
      ...
      if (size == 2) {
        ...
        if (element[i] > 10 && element[i + 1] > 10) {
          ...
          // 3' dangling end:
          energy[i][i + 1] = min(energy[i][i + 1],
              data->dangle[decon2(element[i])][decon1(element[i])][element[i + 1]][1]);

        } else if (element[i + 1] > 10) {
          // 5' dangling end:
          energy[i][i + 1] = min(energy[i][i + 1],
              data->dangle[decon2(element[i + 1])][decon1(element[i + 1])][element[i]][2]);
        }
        ...
        energy[i][i + 3] = min(energy[i][i + 1] + energy[i + 2][i + 3], energy[i][i + 2]);
        ...
        for (j = i; j < i + size - 1; j++) {
          ...
          energy[i][i + size - 1] =
              min(energy[i][i + size - 1], energy[i][j] + energy[j + 1][i + size - 1]);
        }
      }
    }
  }
  ...
  if (intermolecular) return minimum + au * data->auend + data->init;
  ...
  minimum = minimum + (short)round(((float)data->mlasym) * average);
  ...
  if (b > 8 && js && !intermolecular && !simplemb)
    ...
    minimum = minimum + data->efn2a + b * data->efn2b + c * data->efn2c + au * data->auend;
...
void read(ifstream* out, short* i) { out->read((char*)i, sizeof(*i)); }

void read(ifstream* out, bool* i) { out->read((char*)i, sizeof(*i)); }

void read(ifstream* out, int* i) { out->read((char*)i, sizeof(*i)); }
...
void readsinglechar(ifstream* out, char* i) { out->read(i, sizeof(char)); }

void read(ifstream* out, float* i) { out->read((char*)i, sizeof(*i)); }

void read(ifstream* out, double* i) { out->read((char*)i, sizeof(*i)); }

// write is used to write data to a save file
void write(ofstream* out, short* i) { out->write((char*)i, sizeof(*i)); }

void write(ofstream* out, bool* i) { out->write((char*)i, sizeof(*i)); }

void write(ofstream* out, int* i) { out->write((char*)i, sizeof(*i)); }
...
void write(ofstream* out, float* i) { out->write((char*)i, sizeof(*i)); }

void write(ofstream* out, double* i) { out->write((char*)i, sizeof(*i)); }

void writesinglechar(ofstream* out, char* i) { out->write(i, sizeof(char)); }
    ...
    for (b = 0; b < 6; b++) {
      for (c = 0; c < 6; c++) {
        for (d = 0; d < 6; d++) {
          for (e = 0; e < 6; e++) {
            for (f = 0; f < 6; f++) {
              (dg.iloop11[a][b][c][d][e][f]) =
                  Tscale(T, data.iloop11[a][b][c][d][e][f], dhdata.iloop11[a][b][c][d][e][f]);
              for (g = 0; g < 6; g++) {
                (dg.iloop21[a][b][c][d][e][f][g]) = Tscale(
                    T, data.iloop21[a][b][c][d][e][f][g], dhdata.iloop21[a][b][c][d][e][f][g]);

                for (h = 0; h < 6; h++) {
                  (dg.iloop22[a][b][c][d][e][f][g][h]) = Tscale(T,
                      data.iloop22[a][b][c][d][e][f][g][h], dhdata.iloop22[a][b][c][d][e][f][g][h]);
                }
              }
            }
          }
          (dg.tstki1n[a][b][c][d]) =
              Tscale(T, data.tstki1n[a][b][c][d], dhdata.tstki1n[a][b][c][d]);
          (dg.tstki23[a][b][c][d]) =
              Tscale(T, data.tstki23[a][b][c][d], dhdata.tstki23[a][b][c][d]);
          ...
          (dg.coaxstack[a][b][c][d]) =
              Tscale(T, data.coaxstack[a][b][c][d], dhdata.coaxstack[a][b][c][d]);
          (dg.tstackcoax[a][b][c][d]) =
              Tscale(T, data.tstackcoax[a][b][c][d], dhdata.tstackcoax[a][b][c][d]);
