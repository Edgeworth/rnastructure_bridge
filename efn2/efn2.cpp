...
bool efn2Interface::parse(int argc, char** argv) {
  ...
  parser->addOptionFlagsNoParameters(simpleOptions,
  ...
  parser->addOptionFlagsNoParameters(printOptions,
                                     "Print the simple list file to standard output. This won't override default behavior of writing to a file. Thermodynamic files (if written) are not piped.");
  ...
  parser->addOptionFlagsWithParameters(shapeOptions,
                                       "Specify a SHAPE constraints file to be applied. These constraints are pseudoenergy constraints. Default is to have no constraints applied.");
  ...
  parser->addOptionFlagsWithParameters(shapeInterceptOptions,
                                       "Specify an intercept used with SHAPE constraints. Default is -0.6 kcal/mol.");
  ...
  parser->addOptionFlagsWithParameters(shapeSlopeOptions,
                                       "Specify a slope used with SHAPE constraints. Default is 1.8 kcal/mol.");
  ...
  parser->addOptionFlagsNoParameters(detailsOptions,
                                     "Write a thermodynamic details file. The thermodynamic details file replaces the list file that is outputted by default.");
...
void efn2Interface::run() {
  ...
  if (error == 0) {
    ...
    if (writeTherm) {
      ...
      strand->CalculateFreeEnergy(1,
                                  simple);      //calculate the free energy of the first structure first (this is to make it play well with openMP)
#ifdef SMP
#pragma omp parallel for
      ...
      if (error == 0) {
        ...
        for (int i = 1; i <= structures; i++) {
          int index = i - 1;
          out << "Structure: " << i << "   Energy = " << fixed << setprecision(1) << strand->GetFreeEnergy(i)
              << endl; //changed to GetFreeEnergy instead of writing the energies vector
        }
        out.close();
      }
